@if(!str_starts_with($role->system_name ?? '', 'user-proxy-'))
@php
...
<div class="item-list-row flex-container-row py-xs items-center">
    <div class="py-xs px-m flex-2">
        <a href="{{ url("/settings/roles/{$role->id}") }}">{{ $role->display_name }}</a>
        @if($roleTag)
            <span style="background: {{ $roleTag->color }}; color:#fff; font-size:0.7em; padding: 1px 6px; border-radius:3px; margin-left:6px;">{{ $roleTag->tag }}</span>
        @endif
        <br>
