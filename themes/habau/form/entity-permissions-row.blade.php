...
<div component="permissions-table" class="item-list-row flex-container-row justify-space-between wrap">
    <div class="gap-x-m flex-container-row items-center px-l py-m flex">
        <div class="text-large">
            @php
                $isProxy = str_starts_with($role->system_name ?? '', 'user-proxy-');
                $userId = $isProxy ? intval(str_replace('user-proxy-', '', $role->system_name)) : null;
                $proxyUser = $userId ? \BookStack\Users\Models\User::find($userId) : null;
            @endphp
            @if($proxyUser)
                <img src="{{ $proxyUser->getAvatar(32) }}" style="width:32px;height:32px;border-radius:50%;">
            @else
